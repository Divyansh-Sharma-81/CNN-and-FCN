...
def train(args):
    ...
    transform = dense_transforms.Compose([
        ...
        dense_transforms.RandomCrop(256),
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=float(args.learning_rate), weight_decay=0.0002)   
    ...
    for epoch in range(args.num_epochs):
        ...
        # Logging 
        if train_logger:
            train_logger.add_scalar('global_accuracy', acc_val, epoch)
            train_logger.add_scalar('iou', cm.iou, epoch)
        
        # Validation
        model.eval()
        cm_val = ConfusionMatrix(size=5)  # ConfusionMatrix for validation
        with torch.no_grad():
            for img, label in valid_loader:
                ...
                cm_val.add(logits.argmax(1), label)
                print(f"Step {i}, val_global acc: {acc_val:.3f}, val_iou: {iou:}")

        val_global_acc = cm_val.global_accuracy
        val_iou = cm_val.iou

        if valid_logger:
            valid_logger.add_scalar('global_accuracy', val_global_acc, epoch)
            valid_logger.add_scalar('iou', val_iou, epoch)

        print(f'epoch {epoch}, val_global_acc: {val_global_acc}, val_iou: {val_iou}')
